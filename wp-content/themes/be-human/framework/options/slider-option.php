<?php
	...
	*/
	...
	function create_slider() {
		...
		$labels = array(
			'name' => _x('Slider', 'Slider General Name', 'crunchpress'),
			'singular_name' => _x('Slider Item', 'Slider Singular Name', 'crunchpress'),
			'add_new' => _x('Add New', 'Add New Slider Name', 'crunchpress'),
			'add_new_item' => __('Add New Slider', 'crunchpress'),
			'edit_item' => __('Edit Slider', 'crunchpress'),
			'new_item' => __('New Slider', 'crunchpress'),
			'view_item' => '',
			'search_items' => __('Search Slider', 'crunchpress'),
		...
		$args = array(
			...
			'show_ui' => true,
			'query_var' => true,
			'menu_icon' => CP_PATH_URL . '/framework/images/slider-icon.png',
			'rewrite' => true,
	...
	add_action('add_meta_boxes', 'add_slider_option');
	function add_slider_option(){
	
		add_meta_box('cp_slider_option', __('Slider Images','crunchpress'), 'add_slider_option_element',
			'cp_slider', 'normal', 'high');
			
	}
	
	function add_slider_option_element(){
	
		global $post, $slider_meta_box;
	...
	add_action('save_post','save_slider_option_meta');
	function save_slider_option_meta($post_id){
			...
			if( $edit_meta_box['type'] == 'sliderpicker' ){
				...
				if(isset($_POST[$edit_meta_box['name']['image']])){
					...
					for($i=0; $i<=$num; $i++){
						...
						$slider_xml = $slider_xml. create_xml_tag('btn_txt',$btn_new);
						$slider_xml = $slider_xml . "</slider>";
	...
	function print_slider_picker($args){
				...
				<div class="meta-input-slider">
					<div class="image-picker" id="image-picker">
						...
						<div class="selected-image" id="selected-image">
							<div id="selected-image-none"><?php _e('No Image Inserted', 'crunchpress'); ?></div>
							<ul>
										...
										<div class="meta-input meta-detail-input">
											<div class="combobox">
												<select id='<?php echo $name['linktype']; ?>'>
													<option><?php _e('No Link', 'crunchpress'); ?></option>
													<option selected><?php _e('Lightbox', 'crunchpress'); ?></option>
													<option><?php _e('Link to URL', 'crunchpress'); ?></option>	
												</select>
											</div>
										</div><br class="clear">
										<div class="url">
											<div class="meta-title meta-detail-title" rel="url"><?php _e('URL PATH', 'crunchpress'); ?></div> 
											<div class="meta-detail-input meta-input"><input class="mt10" type="text"  id='<?php echo $name['link']; ?>' /></div>
										</div>
										<hr class="separator">
										<div class="meta-title meta-detail-title"><?php _e('SLIDER BUTTON TEXT', 'crunchpress'); ?></div> 
													...
													<div class="meta-input meta-detail-input">
														<div class="combobox">
															...
															<select name='<?php echo $name['linktype']; ?>[]' id='<?php echo $name['linktype']; ?>' >
																<option <?php echo ($linktype_val == 'No Link')? "selected" : ''; ?> ><?php _e('No Link', 'crunchpress'); ?></option>
																<option <?php echo ($linktype_val == 'Lightbox')? "selected" : ''; ?>><?php _e('Lightbox', 'crunchpress'); ?></option>
																<option <?php echo ($linktype_val == 'Link to URL')? "selected" : ''; ?>><?php _e('Link to URL', 'crunchpress'); ?></option>
															</select>
														</div>
													</div><br class="clear">
													<div class="url">
														<div class="meta-title meta-detail-title" rel="url"><?php _e('URL PATH', 'crunchpress'); ?></div> 
														<div class="meta-detail-input meta-input"><input class="mt10" type="text" name='<?php echo $name['link']; ?>[]' id='<?php echo $name['link']; ?>[]' value="<?php echo esc_url(find_xml_value($slider, 'link')); ?>" /></div>
													</div>
													<div class="clear"></div>
													<div class="meta-title meta-detail-title"><?php _e('BUTTON TEXT', 'crunchpress'); ?></div> 
													<div class="meta-detail-input meta-input"><input class="" type="text" name='<?php echo $name['btn_txt']; ?>[]' id='<?php echo $name['btn_txt']; ?>[]' value="<?php echo find_xml_value($slider, 'btn_txt'); ?>" /></div>
