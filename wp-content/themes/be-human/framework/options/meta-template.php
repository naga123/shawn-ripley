	...
	*/
	...
	function print_meta($meta_box){
		...
		switch($meta_box['type']){
			...
			case "combobox_category": print_meta_input_combobox($meta_box); break;
		...
		?>
			...
			<div class="meta-body">
				...
				<div class="meta-input">	
					<div class="combobox">
						<select name="<?php echo $name; ?>" id="<?php echo str_replace('[]', '', $name); ?>">
							<option value="0" rel="0" <?php if( esc_html($value) == '0' ){ echo 'selected'; }?> ><?php echo __('All','crunchpress'); ?></option>
							<?php foreach($options as $option){ ?>
								<option value="<?php echo $option->term_id ; ?>" rel="<?php echo $option->term_id ; ?>" <?php if( $option->ID == esc_html($value) ){ echo 'selected'; }?> ><?php echo $option->name ; ?></option>
							<?php } ?>
						</select>
					</div>
				</div>
