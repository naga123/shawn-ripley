...
function reg_shortcode_button($buttons)
{
  ...
  /*  array_push($buttons, "|", "tab-shortcodes");*/
	...
	array_push($buttons, "", "social-shortcodes");
    return $buttons;
}
// load the js file
function add_shortcode_tinymce_plugin($plugin_array)
{
    $plugin_array['cp_themeshortcode'] = CP_FW_URL . '/extensions/shortcodes/shortcodes.js';
    return $plugin_array;
}
// Refresh the editor 
        ...
        if (empty($type)) {
            foreach ($slider_xml_dom->documentElement->childNodes as $slider) {
                ...
                if ($link_type == 'Link to URL') {
                    ...
                    $cp_gallery = $cp_gallery . '<a href="prettyPhoto[bkpGallery' . $galid . ']" href="' . $image_full[0] . '"  title="">';
                    $cp_gallery = $cp_gallery . '<img class="cp-gallery-image" src="' . $image_url[0] . '" alt="' . $alt_text . '" />';
                    $cp_gallery = $cp_gallery . '</a>';
                } else {
                    $cp_gallery = $cp_gallery . '<img class="cp-gallery-image" src="' . $image_url[0] . '" alt="' . $alt_text . '" />';
            ...
            $cp_gallery = $cp_gallery . '<div class="clear"></div>';
...
function cp_accordion_shortcode($atts, $content = null)
{
    ...
    $accordion = "<div class='accordion_cp'>";
...
function cp_acc_item_shortcode($atts, $content = null)
{
    ...
    $acc_item = "";
    $acc_item = $acc_item . "<h3 class='accordion-heading'><a href=''>";
...
function cp_tab_shortcode($atts, $content = null)
{
    ...
    $tab = "<div id='horizontal-tabs' class='tabs'><ul class='tab_content'>";
    for ($i = 0; $i < $num; $i++) {
        $active = ($i == 0) ? 'active' : '';
        $tab_id = str_replace(' ', '-', $cp_tab_array[$i]["title"]);
        $tab    = $tab . '<li><a href="#' . $tab_id . '" class=" ';
    ...
    for ($i = 0; $i < $num; $i++) {
        ...
        $tab    = $tab . $active . '" >' . $cp_tab_array[$i]["content"] . '</li>';
    }
    $tab = $tab . "</ul></div>";
    return $tab;
}
...
function cp_social_shortcode($atts, $content = null)
{
    extract(shortcode_atts(array(        
		...
		$social = '<div class="space_btwn" id="socialicons">';
		$social = $social . '<a title="'.$content.'" href="'.$src.'" class="'.$size.' social_active" id="'.$type.'"><span class="da-animate da-slideFromLeft"></span></a></div>';
    return $social;
}
...
function cp_frame_shortcode($atts)
{
    ...
    $frame  = "<img class='pic' src='" . $src . "' style='width:" . $width . "; height:" . $height . ";' alt='' />";
    if ($lightbox == 'on') {
        $frame = "<div class='shortcode_pop'><a href='" . $src . "' rel='prettyPhoto[image]' alt=''>" . $frame . "<span class='hover-effect big zoom'></span></a></div>";
    }
...
function cp_price_item_shortcode($atts)
{
    ...
    $price_item  = '<div class="cp-price-item">';
    ...
    foreach ($price_posts as $price_post) {
        ...
        $price_item = $price_item . '<div class="percent-column1-' . $item_number . ' cp-divider">';
